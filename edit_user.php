    ...
    if (empty($contact)) 
    {
  	...
  	if (empty($errors)) 
  	{
  		if(empty($password))
  		{
	  		...
	  		$pdo->prepare($sql)->execute([$name, $contact, $email, $id]);
	  	}
	  	else
	  	{
	  		...
	  		$pdo->prepare($sql)->execute([$name, $contact, $email, $password, $id]);
	  	}
	...
	<div class="col-md-4">
		...
		<div class="card">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="contact" name="contact" placeholder="Enter Contact" value="<?php echo isset($user['contact']) ? $user['contact'] : ''; ?>">
            </div>
				  	<div class="mb-3">
